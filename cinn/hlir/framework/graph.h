...
class Graph : public cinn::common::Graph {
 public:
  ...
  struct Group {
   ...
   public:
    const std::unordered_set<std::shared_ptr<Group>>& producer_groups() const {
      return producer_groups_;
    }

    const std::unordered_set<std::shared_ptr<Group>>& consumer_groups() const {
      return consumer_groups_;
    }

    std::unordered_set<std::shared_ptr<Group>>* mut_producer_groups() {
      return &producer_groups_;
    }

    std::unordered_set<std::shared_ptr<Group>>* mut_consumer_groups() {
      return &consumer_groups_;
    }
   ...
   private:
    // input groups
    std::unordered_set<std::shared_ptr<Group>> producer_groups_;
    // output grous
    std::unordered_set<std::shared_ptr<Group>> consumer_groups_;
  };
